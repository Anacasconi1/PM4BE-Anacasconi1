...
export class ProductsController {
  ...
  @Get()
  async findAll(@Query('limit') limit?: string, @Query('page') page?:string): Promise <Product[]>{
    return await this.productsService.getAllProducts(Number(limit),Number( page));
  }
  ...
  async UpdateProduct(@Param('id', ParseUUIDPipe) id: string, ProductDto){
    const ProductUpdated = await this.productsService.updateProduct(id, ProductDto)
  ...
  @Get(':id')
  async GetProductById (@Param('id', ParseUUIDPipe) id: string):Promise<Product | string>{
